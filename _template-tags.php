<?php
/**
 * Custom template tags for Ignition Press
...
if ( ! function_exists( 'ign_posted_by' ) ) {
	function ign_posted_by() {
		...
		$author_name        = sprintf( __( '%s by %s', 'ignitionpress' ), '<a href="' . $author_link . '" class="author-name"><span class="byline">', '</span>' . get_the_author() . '</a>' );
	...
	function ign_get_term_links( $taxonomy = 'category', $get_all = false, $id = 0 ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) :
			...
			if ( ! $get_all ) {
				...
				$term_links_output = join( '<span class="delim">' . __( ', ', 'ignitionpress' ) . '</span>', $term_links );
			}
	...
	function ign_edit_link( $id = null, $class = '', $text = 'Edit' ) {
		...
		edit_post_link(
			...
			/* translators: %s: Name of current post */
				__( '%s<span class="screen-reader-text"> "%s"</span>', 'ignitionpress' ),
				$text, get_the_title()
			),
